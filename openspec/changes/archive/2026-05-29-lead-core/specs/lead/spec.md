...
本 capability 在 lead-core change 阶段 SHALL **不**提供以下能力（这些属于后续 lead-ownership / lead-stage / lead-closure change，将向本 capability 增量加入对应 requirement）：

- 公海视图（Sales 看公海列表 + 脱敏联系电话）
- Sales 认领公海线索 / Sales 退回公海
- Admin 分配 / 回收 / 转移
- 阶段变更（PATCH /stage）
- 标记赢单 / 标记流失 + 合同记录生成
- 编辑 / 删除既有线索（MVP 整体不支持）

#### Scenario: lead-core 阶段不存在认领端点

- **WHEN** 任意已认证用户对 `POST /api/leads/{id}/claim` 发起请求（lead-core 阶段）
- **THEN** 系统**未**对外暴露此端点（具体响应为 `404 NOT_FOUND` 或 `405 METHOD_NOT_ALLOWED`，**不**为 `SUCCESS`）

#### Scenario: lead-core 阶段不存在阶段变更端点

- **WHEN** 任意已认证用户对 `PATCH /api/leads/{id}/stage` 发起请求（lead-core 阶段）
- **THEN** 系统**未**对外暴露此端点（响应**不**为 `SUCCESS`）

#### Scenario: lead-core 阶段不存在赢单 / 流失端点

- **WHEN** 任意已认证用户对 `POST /api/leads/{id}/win` 或 `POST /api/leads/{id}/lose` 发起请求（lead-core 阶段）
- **THEN** 系统**未**对外暴露此端点
